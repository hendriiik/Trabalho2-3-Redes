...
int crcCheck(char* in, char* polyn, char* check){
    char* in2=malloc(strlen(in)+strlen(check));
    strcpy(in2,in);
    strcat(in2,check);
    printf("%s\n",in2);
    int i=0;
...
unsigned int CRC32Wiki(unsigned int data[], size_t dataLen){ //wikipedia
    ...
    for (size_t i=0; i<dataLen; ++i){
        unsigned int lookup = (crc32 ^ data[i]) & 0xff;
        crc32 = (crc32 >> 8) ^ CRCTable[lookup];
    }
...
unsigned int crc32(const char* s,size_t n) {
	...
	for(size_t i=0;i<n;i++) {
		...
		for(size_t j=0;j<8;j++) {
			...
			crc>>=1;
			if(b) crc=crc^0xEDB88320; //lsb first //0x04C11DB7 //msb first
			ch>>=1;
		}
	}
...
char* nPadString(int n, char* msg){
    char* msg2=malloc(strlen(msg)+n);
    strcpy(msg2,msg);
    //msg=malloc(strlen(msg));
    //msg=msg2;
    //msg=malloc(1);
    ///msg=malloc(strlen(msg)+n);
    //strcat(msg2,msg);
    while (n--){
        strcat(msg2,"0");
    }
